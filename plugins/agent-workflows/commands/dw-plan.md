...
checkpoint a fresh session resumes from. See `dev-workflow`.
...
The plan issue. Reported per `agent-report` — the verdict first, and a
...
- The plan issue is the parent of the task issues; the story records the need, the plan
  records the approach, the task issues carry the how (see `dev-workflow`)
